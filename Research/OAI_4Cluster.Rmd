...
data_k4 <- getCompleteData(DATAPATH, "K.4.Clusters")
data_full <- data_k4
remove(data_k4)
```
...
```         
Residual Deviance: 8198.342
AIC: 8414.342
P-Val of Dev Diff: 0.4109241
```
...
pchisq(dev_bac-dev_full, df_full - df_bac)
```

-   Backward Selection

```         
Residual Deviance: 8180.658
AIC: 8396.658
P-Val of Dev Diff: 0.1225218
```
...
[13] "EDCV_HSDeg1"       
[14] "P01OAGRD_Severe1"  
[15] "P01OAGRD_Moderate1"
[16] "P01OAGRD_Mild1"    
[17] "P01OAGRD_Possible1"
[18] "RACE_AA1"
```
...
pchisq(dev_both-dev_full, df_full - df_both)
```
...
predictors_best <- c("AGE", "SEX", "RACE_AA", "RACE_NW", "CEMPLOY_NWH", "PASE", "WOMKP","WOMSTF", "BMI", "WEIGHT", "NSAID", "P01OAGRD_Severe", "P01OAGRD_Moderate", "P01OAGRD_Mild", "P01OAGRD_Possible", "EDCV_HSDeg","EDCV_GradDeg", "EDCV_UGDeg", "V00WTMAXKG", "MEDINS")
...
```
```{r, eval=FALSE}
dev_best <- deviance(mod_best)
df_best <- mod_best$edf
pchisq(dev_best-dev_full, df_full - df_best)
```

```         
Residual Deviance: 8144.553
AIC: 8396.553
P-Val of Dev Diff: .0004302032
```
...
```{r probPredit}
new_data = data.frame(AGE=50, SEX="1", RACE_AA="0", RACE_NW="0", CEMPLOY_NWH="0", PASE=0, WOMKP=0,WOMSTF=0, BMI=0, WEIGHT=0, NSAID="0", P01OAGRD_Severe="0", P01OAGRD_Moderate="0", P01OAGRD_Mild="0", P01OAGRD_Possible="0", EDCV_HSDeg="0",EDCV_GradDeg="0", EDCV_UGDeg="0", V00WTMAXKG=0, MEDINS="0")
...
for (i in seq(2,nrow(odds)*2,2)) {
  ...
  plt <- ggplot(odds_df,  aes(x = boxOdds, y = predictors_best)) + 
    ...
    odds_df <- data.frame(boxOdds = log10(odds[i/2,-1]),
                        boxCILow = log10(odds_ci[,1,i/2][-1]),
                        boxCIHigh = log10(odds_ci[,2,i/2][-1]))
    ci_min = log10(ci_min)
    ci_max = log10(ci_max)
    
    plt <- ggplot(odds_df,  aes(x = boxOdds, y = predictors_best)) + 
    ylab("Predictor") +
    xlab("Log Base 10 Odds Ratio") +
