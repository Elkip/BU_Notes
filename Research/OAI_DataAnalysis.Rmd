...
```{r}
ggplot(data_full, aes(SEX, fill = EVNT)) + geom_bar()
```
...
```{r}
ggplot(data_full, aes(RACE_O, fill = EVNT)) + geom_bar()
```
...
data_full %>% 
    ...
    add_row(EVNT = "0", freq = nrow(data_full), 
            NW = sum(data_full$RACE_O == 1), NW_perc = NW/freq*100,
            W = freq - NW, W_perc = W / freq*100) %>%
    mutate(across(where(is.numeric), round, 2))
```
...
```{r}
ggplot(data_full, aes(ETHNICITY, fill = EVNT)) + geom_bar()
```
...
```{r}
ggplot(data_full, aes(MEDINS, fill = EVNT)) + geom_bar()
```
...
```{r}
par(mfrow = c(1, 2))
hist(data_full$WOMADL)
hist(log(data_full$WOMADL + 1))
par(mfrow = c(1,1))
```
...
```{r}
par(mfrow = c(1, 2))
hist(data_full$WOMKP)
hist(log(data_full$WOMKP + 1))
par(mfrow = c(1,1))
```
...
```{r}
par(mfrow = c(1, 2))
hist(data_full$WOMSTF)
hist(log(data_full$WOMSTF + 1))
par(mfrow = c(1,1))
```
...
```{r}
par(mfrow = c(1, 2))
hist(data_full$V00WTMINKG)
hist(log(data_full$V00WTMINKG))
par(mfrow = c(1,1))
```
...
```{r}
ggplot(data_full, aes(CEMPLOY_NW, fill = EVNT)) + geom_bar()
```
...
```{r}
ggplot(data_full, aes(NSAID, fill = EVNT)) + geom_bar()
```
...
```{r}
ggplot(data_full, aes(NARC, fill = EVNT)) + geom_bar()
```
...
```{r}
par(mfrow = c(1, 2))
hist(data_full$COMORBSCORE)
hist(log(data_full$COMORBSCORE))
par(mfrow = c(1,1))
```
...
```{r}
ggplot(data_full, aes(DPRSD, fill = EVNT)) + geom_bar()
```
...
```{r}
ggplot(data_full, aes(Surg_Inj_Hist, fill = EVNT)) + geom_bar()
```
