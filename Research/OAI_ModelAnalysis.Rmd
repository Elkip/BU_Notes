...
if(!exists('data_full')) {
  ...
  data_k5 <- getCompleteData(DATAPATH, "K.5.Clusters", exportSAS = TRUE)
  data_full <- data_k5
  remove(data_k5)
  remove(bsln)
  remove(evnt)
...
```{r probPlotAvgs}
avg_data <- data_best %>% 
  group_by(EVNT, SEX, RACE_O, NSAID, P01OAGRD_Severe, P01OAGRD_Moderate,
  ...
  summarise(AGE = mean(AGE), CESD = mean(CESD), PASE = mean(PASE), WOMKP = mean(WOMKP),
            WOMSTF = mean(WOMSTF), HEIGHT = mean(HEIGHT), WEIGHT = mean(WEIGHT), 
...
probPlot <- function(term) {
  new_data %>%
    pivot_longer(22:28, names_to = "Cluster", values_to = "prob") %>%
