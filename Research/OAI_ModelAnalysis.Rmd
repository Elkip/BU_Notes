...
predictors_best <- c("AGE", "SEX", "RACE_O", "PASE", "WOMKP", "WOMSTF", "BMI", "WEIGHT", "V00WTMAXKG", "NSAID", "P01OAGRD_Severe", "P01OAGRD_Moderate", "P01OAGRD_Mild", "P01OAGRD_Possible", "EDCV_HSDeg", "EDCV_GradDeg", "EDCV_UGDeg", "CESD", "Surg_Inj_Hist")
...
# Best Model Analysis - 8 Cluster Outcome
...
```{r testProb}
new_data = data.frame(AGE=50, SEX="1", RACE_O="0", CESD=0, CEMPLOY_NW="0", PASE=0, WOMKP=0,WOMSTF=0, BMI=0, WEIGHT=0, NSAID="0", P01OAGRD_Severe="0", P01OAGRD_Moderate="0", P01OAGRD_Mild="0", P01OAGRD_Possible="0", EDCV_HSDeg="0", EDCV_GradDeg="0", EDCV_UGDeg="0", V00WTMAXKG=0, Surg_Inj_Hist="0")
...
```{r probPlotAvgs}
avg_data <- data_full %>% 
  group_by(EVNT, SEX, RACE_O, CEMPLOY_NW, NSAID, P01OAGRD_Severe, P01OAGRD_Moderate,
           P01OAGRD_Mild, P01OAGRD_Possible, EDCV_GradDeg, EDCV_UGDeg, EDCV_HSDeg,
           Surg_Inj_Hist) %>%
  summarise(AGE = mean(AGE), CESD = mean(CESD), PASE = mean(PASE), WOMKP = mean(WOMKP),
            WOMSTF = mean(WOMSTF), BMI = mean(BMI), WEIGHT = mean(WEIGHT), 
            V00WTMAXKG = mean(V00WTMAXKG))
...
probPlot <- function(term) {
  new_data %>%
    pivot_longer(22:29, names_to = "Cluster", values_to = "prob") %>%
