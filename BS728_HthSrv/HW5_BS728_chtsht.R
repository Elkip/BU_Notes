...
# read in the data
deaths <- read.csv("LOCATION OF DATA/deaths.csv",header=T)
...
# plot a residual plot for the model with only the annual term
fit1 <- lm(num~s1+c1)

plot(num,type='l')
lines(fit1$fitted.values,col="blue",lwd=3)

plot(fit1$residuals,type='l')

# plot a residual plot for the model with the semi-annual term
...
fit.lin <- lm(resids.sinusoid~time)
summary(fit.lin) 
...
time2 <- time^2
# <fill in the rest of the code to fit the quadratic model here>
