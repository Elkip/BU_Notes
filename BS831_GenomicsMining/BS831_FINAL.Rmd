...
library(Biobase)
# library(BS831) # Fuck your stupid library
...
```{r}
## read CPM data
CPM <- readRDS( file.path(DATA,"HNSC_htseq_normalized_AEvsG1vsG3.RDS") )
...
hall <- read.gmt( file.path(DATA,"h.all.v6.1.symbols.gmt") )
...
p.list <- c()
for (i in 1:50) {
  geneIDX <- c(match(hall[[i]], genesSorted))
