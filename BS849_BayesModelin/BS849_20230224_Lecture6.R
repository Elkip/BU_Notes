...
par(mfrow=c(1,3),mar=c(3.5,3,0.05,0.5),mgp=c(2.3,0.7,0),cex.lab=0.9,cex.axis=0.9)
set.seed(10)
   x <- rnorm(100, 0,1)
   y <- rnorm(300, 2,1)
...
par(mfrow=c(1,1),mar=c(3.7,3,0.05,0.5),mgp=c(1.5,0.5,0),cex.lab=0.5,cex.axis=0.5, tck=-0.02)
set.seed(10)
   x <- rnorm(100, 0,1)
   y <- rnorm(300, 4,1)
   z <- c(x,y) 
...
library(rjags)
jags.1 <- jags.model(textConnection(model.1),data=data.sim1, n.adapt=1500)
update(jags.1, 10000)
test.1 <- coda.samples(jags.1, c('mu','sigma','theta','epsilon'),
...
## ----L6S7_4, message=FALSE, warning=FALSE--------------------------------
   geweke.diag(test.1[, c("mu[1]","mu[2]","theta")], frac1=0.1, frac2=0.5)


## ----L6S7_5, echo = FALSE, message=FALSE, warning=FALSE------------------
summ.test1 <- summary(test.1[, c("mu[1]","mu[2]","theta")])

mu1 <- round(as.vector(summ.test1[[2]]["mu[1]", c("2.5%", "50%", "97.5%")]), 2)
mu2 <- round(as.vector(summ.test1[[2]]["mu[2]", c("2.5%", "50%", "97.5%")]), 2)
theta <- round(as.vector(summ.test1[[2]]["theta", c("2.5%", "50%", "97.5%")]), 2) * 100


## ----L6S8_1, echo = FALSE, message=FALSE, warning=FALSE, fig.align="center",fig.width=2,fig.height=3----
par(mfrow=c(2,1),mar=c(2,1,1,0),mgp=c(.5,0.2,0),cex.lab=0.5,cex.axis=0.5,cex.main=0.5, tck=-0.03)
...
## ----L6S10_2a, message=FALSE, warning=FALSE, fig.align="center",fig.width=3,fig.height=1.6,echo=F----
   par(mfrow=c(1,1),mar=c(2,2,1,0),mgp=c(.65,0.2,0),cex.lab=0.5,cex.axis=0.5,cex.main=0.5, tck=-0.03)
   ind.a <-which(apply(res.epsilon,2,median)==1) # Group 1
   plot(c(1:400)[-ind.a],z[-ind.a],col=3,cex=0.5,xlab='Index',ylab='',ylim=range(z))
   points(c(1:400)[ind.a],z[ind.a],col=2,pch=22,bg='gray',cex=0.5)
...
jags.1 <- jags.model(textConnection(model.1),data=data.sim2, n.adapt=1500)
update(jags.1,10000)
test.1 <- coda.samples(jags.1, c('mu','sigma','theta','epsilon'), n.adapt=1500, n.iter=10000)
...
par(mfrow=c(2,1),mar=c(1,1,1,0),mgp=c(.5,0.2,0),cex.lab=0.4,cex.axis=0.4,cex.main=0.4, tck=-0.02)
   densplot(test.1[, c("epsilon[1]", "epsilon[2]")], col=2)
...
par(mfrow=c(1,1),mar=c(3.7,3,0.05,0.5),mgp=c(1,0.5,0),cex.lab=0.5,cex.axis=0.5, tck=-0.02)
set.seed(5)
data <- read.csv("data.change.csv", header=T)
data <- data[sample(c(1:nrow(data)),500),]
data.list <- list(N=nrow(data), gr.s =data$grip.strength.b, ga.s=data$gait.b )
...
set.seed(5)
jags.1 <- jags.model(textConnection(model.1),data=data.list, n.adapt=1500)
update(jags.1, 1500)
test.1 <- coda.samples(jags.1, c('mu.grs','mu.gas','theta','epsilon',"tau.grs","tau.gas"), n.iter=1500)


## ----L6S18_1, echo = FALSE, message=FALSE, warning=FALSE, fig.align="center",fig.width=4,fig.height=1.7----
  par(mfrow=c(2,3),mar=c(2,1,1,0),mgp=c(0.6,0.1,0),cex.lab=0.5,cex.axis=0.5,cex.main=0.5, tck=-0.02)
...
## ----L6S18_2, echo = FALSE, message=FALSE, warning=FALSE, fig.align="center",fig.width=4,fig.height=1.7----
  par(mfrow=c(2,3),mar=c(2,1,1,0),mgp=c(0.6,0.1,0),cex.lab=0.4,cex.axis=0.4,cex.main=0.4, tck=-0.02)
...
## ----L6S19_1, echo = FALSE, message=FALSE, warning=FALSE-----------------
   summary(test.1[, c("mu.grs[1]","mu.grs[2]","tau.grs[1]","tau.grs[2]","mu.gas[1]","mu.gas[2]","tau.gas[1]","tau.gas[2]","theta")])
   geweke.diag(test.1[, c("mu.grs[1]","mu.grs[2]","mu.gas[1]","mu.gas[2]","theta")], frac1=0.1, frac2=0.5)

summ.test.1 <- summary(test.1[, c("mu.grs[1]","mu.grs[2]","tau.grs[1]","tau.grs[2]","mu.gas[1]","mu.gas[2]","tau.gas[1]","tau.gas[2]","theta")])
  summ.epsilon <- summary(test.1[, c("epsilon[1]","epsilon[2]","epsilon[3]","epsilon[5]")])#grep("epsil", names(temp))
  epsilon1 <- mean(unlist(test.1[, c("epsilon[1]")])==1)#grep("epsil", names(temp))

grs1 <- round(as.vector(summ.test.1[[2]]["mu.grs[1]", c("2.5%", "50%", "97.5%")]), 2)
gas1 <- round(as.vector(summ.test.1[[2]]["mu.gas[1]", c("2.5%", "50%", "97.5%")]), 2)
tau.grs1 <- round(as.vector(summ.test.1[[2]]["tau.grs[1]", c("2.5%", "50%", "97.5%")]), 2)
tau.gas1 <- round(as.vector(summ.test.1[[2]]["tau.gas[1]", c("2.5%", "50%", "97.5%")]), 2)

grs2 <- round(as.vector(summ.test.1[[2]]["mu.grs[2]", c("2.5%", "50%", "97.5%")]), 2)
gas2 <- round(as.vector(summ.test.1[[2]]["mu.gas[2]", c("2.5%", "50%", "97.5%")]), 2)
tau.grs2 <- round(as.vector(summ.test.1[[2]]["tau.grs[2]", c("2.5%", "50%", "97.5%")]), 2)
tau.gas2 <- round(as.vector(summ.test.1[[2]]["tau.gas[2]", c("2.5%", "50%", "97.5%")]), 2)
...
par(mfrow=c(2,1),mar=c(3,1,1.2,0),mgp=c(.5,0.1,0),cex.lab=0.4,cex.axis=0.4,cex.main=0.4, tck=-0.02)
hist(data$grip.strength.b,main='',xlab='Grip Strength',probability = T,)
x<-seq(0,70,length=1000)
lines(x,theta[2]/100*dnorm(x,grs1[2],sqrt(1/tau.grs1[2]))+(1-theta[2]/100)*dnorm(x,grs2[2],sqrt(1/tau.grs2[2])))
hist(data$gait.b,main='',xlab='Gait Speed',probability = T,ylim=c(0,1.35))
x<-seq(0,2,length=1000)
lines(x,theta[2]/100*dnorm(x,gas1[2],sqrt(1/tau.gas1[2]))+(1-theta[2]/100)*dnorm(x,gas2[2],sqrt(1/tau.gas2[2])))


## ----L6S21_1, echo = FALSE, message=FALSE, warning=FALSE, fig.align="center",fig.width=4.5,fig.height=2.5----
par(mfrow=c(1,3),mar=c(3.5,1,1.5,0),mgp=c(1,0.2,0),cex.lab=0.5,cex.axis=0.5,cex.main=0.5, tck=-0.02)
   densplot(test.1[, c("epsilon[1]", "epsilon[2]","epsilon[5]")], col=2)
...
jags.1 <- jags.model(textConnection(model.1),data=data.list, n.adapt=1500)
update(jags.1,1500)
test.1 <- coda.samples(jags.1, c('mu.grs','mu.gas', 'theta'),
...
grs3 <- round(as.vector(summ.test.1[[2]]["mu.grs[3]", c("2.5%", "50%", "97.5%")]), 2)
gas3 <- round(as.vector(summ.test.1[[2]]["mu.gas[3]", c("2.5%", "50%", "97.5%")]), 2)

theta1 <- round(as.vector(summ.test.1[[2]]["theta[1]", c("2.5%", "50%", "97.5%")]), 2) * 100
theta2 <- round(as.vector(summ.test.1[[2]]["theta[2]", c("2.5%", "50%", "97.5%")]), 2) * 100
...
par(mfrow=c(1,2),mar=c(3.5,1,1.5,0),mgp=c(1,0.2,0),cex.lab=0.5,cex.axis=0.5,cex.main=0.5, tck=-0.02)
